...
Pre_AGWA_Settings:
  cmd.run:
    ...
    - shell: /bin/bash
    - unless: /bin/bash -c "[ -f "${TETHYS_PERSIST}/agwa_setup_complete" ];"
...
Link_Persistent_Stores_Database:
  cmd.run:
    - name: ". {{ CONDA_HOME }}/bin/activate {{ CONDA_ENV_NAME }} && tethys link persistent:{{ PS_SERVICE_NAME }} agwa:ps_database:primary_db"
    - shell: /bin/bash
    - unless: /bin/bash -c "[ -f "${TETHYS_PERSIST}/agwa_setup_complete" ];"

Link_Persistent_Stores_Connection:
  cmd.run:
    - name: ". {{ CONDA_HOME }}/bin/activate {{ CONDA_ENV_NAME }} && tethys link persistent:{{ PS_SERVICE_NAME }} agwa:ps_connection:model_db_1"
    - shell: /bin/bash
    - unless: /bin/bash -c "[ -f "${TETHYS_PERSIST}/agwa_setup_complete" ];"

Link_Existing_Model_Databases:
  cmd.run:
    - name: ". {{ CONDA_HOME }}/bin/activate {{ CONDA_ENV_NAME }} && agwa linkdbs"
    - shell: /bin/bash
    - unless: /bin/bash -c "[ -f "${TETHYS_PERSIST}/agwa_setup_complete" ];"

Link_Spatial_Dataset_Service:
  cmd.run:
    - name: ". {{ CONDA_HOME }}/bin/activate {{ CONDA_ENV_NAME }} && tethys link spatial:{{ GS_SERVICE_NAME }} agwa:ds_spatial:primary_geoserver"
    - shell: /bin/bash
    - unless: /bin/bash -c "[ -f "${TETHYS_PERSIST}/agwa_setup_complete" ];"
...
Init_AGWA:
  cmd.run:
    - name: ". {{ CONDA_HOME }}/bin/activate {{ CONDA_ENV_NAME }} && agwa init {{ TETHYS_GS_PROTOCOL }}://{{ TETHYS_GS_USERNAME }}:{{ TETHYS_GS_PASSWORD }}@{{ TETHYS_GS_HOST }}:{{ TETHYS_GS_PORT }}/geoserver/rest/"
    - shell: /bin/bash
    - unless: /bin/bash -c "[ -f "${TETHYS_PERSIST}/agwa_setup_complete" ];"
...
Flag_Complete_Setup:
  cmd.run:
    - name: touch ${TETHYS_PERSIST}/agwa_setup_complete
    - shell: /bin/bash
